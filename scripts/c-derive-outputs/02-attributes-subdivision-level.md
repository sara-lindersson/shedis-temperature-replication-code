Attributes at the subdivision-level
================
Sara Lindersson
2025-03-04

This script derives attributes at the administrative *subdivision* unit
level for each disaster number *disno*. The outputs from this script
include:  
+ `shedis_heatwaves_subdivision.csv`: This file contains the derived
data attributes for the heat wave disasters.  
+ `shedis_heatwaves_subdivision.gpkg`: This file includes both the data
attributes and a simplified polygon of the corresponding administrative
subdivision.  
+ `shedis_coldwaves_subdivision.csv`: This file contains the derived
data attributes for the cold wave disasters.  
+ `shedis_coldwaves_subdivision.gpkg`: This file includes both the data
attributes and a simplified polygon of the corresponding administrative
subdivision.
...
process_attributes <- function(disno, gadm_gid, analysis_start, analysis_end) {
...
general_attributes <- subnat %>%
  ...
  select(disno, gadm_gid, analysis_start, analysis_end) %>%
  pmap_dfr(function(disno, gadm_gid, analysis_start, analysis_end) {
    process_attributes(disno, gadm_gid, analysis_start, analysis_end)
  })
```
...
process_events10 <- function(disno, gadm_gid, type){
  ...
  if (file.exists(file_path)) {
    ...
    if (nrow(df) > 0){
      ...
      result10 <- cbind(uniquepoints, event_attr, ndays) %>%
        mutate(
          disno = disno,
          gadm_gid = gadm_gid
        ) %>%
...
event_attributes10 <- subnat %>%
  as.data.frame() %>%
  mutate(geometry = NULL) %>%
  select(disno, gadm_gid, type) %>%
  pmap_dfr(function(disno, gadm_gid, type) {
    process_events10(disno, gadm_gid, type)
  })


# Now do the same thing but for the 5th and 95th percentiles
# Function to process each file if it exists
process_events5 <- function(disno, gadm_gid, type){
  ...
  if (file.exists(file_path)) {
    ...
    if (nrow(df) > 0){
      ...
      result5 <- cbind(uniquepoints, event_attr, ndays) %>%
        mutate(
          disno = disno,
          gadm_gid = gadm_gid
        ) %>%
...
# Apply the process_file5 function to each row in subnat
event_attributes5 <- subnat %>%
  as.data.frame() %>%
  mutate(geometry = NULL) %>%
  select(disno, gadm_gid, type) %>%
  pmap_dfr(function(disno, gadm_gid, type) {
    process_events5(disno, gadm_gid, type)
  })
```
...
shedis_coldwaves_subdivision <- output %>%
...
shedis_heatwaves_subdivision <- output %>%
...
shedis_coldwaves_subdivision <- shedis_coldwaves_subdivision %>%
...
shedis_heatwaves_subdivision <- shedis_heatwaves_subdivision %>%
  mutate(country = countrycode(country, origin = "country.name", destination = "country.name.en")) %>%
  arrange(iso3c, disno) %>% 
  rename(subtype = type,
...
st_write(
  shedis_coldwaves_subdivision,
  dsn = paste0(ph_out_cw, '/shedis_coldwaves_subdivision.gpkg'),
  layer = 'shedis_coldwaves_subdivision',
  delete_dsn = TRUE
)

st_write(
  shedis_heatwaves_subdivision,
  dsn = paste0(ph_out_hw, '/shedis_heatwaves_subdivision.gpkg'),
  layer = 'shedis_heatwaves_subdivision',
  delete_dsn = TRUE
)

# Export outputs as CSV (no geometries)
shedis_coldwaves_subdivision_csv <- shedis_coldwaves_subdivision %>%
  as.data.frame() %>%
  mutate(geometry = NULL)

write.csv(
  shedis_coldwaves_subdivision_csv,
  paste0(ph_out_cw, '/shedis_coldwaves_subdivision.csv'),
  row.names = F
)

shedis_heatwaves_subdivision_csv <- shedis_heatwaves_subdivision %>%
  as.data.frame() %>%
  mutate(geometry = NULL)

write.csv(
  shedis_heatwaves_subdivision_csv,
  paste0(ph_out_hw, '/shedis_heatwaves_subdivision.csv'),
  row.names = F
)
```
