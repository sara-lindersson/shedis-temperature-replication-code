Attributes on administrative unit level
================
Sara Lindersson
2024-10-14

This script derives attributes at the disaster record level. The outputs
from this script include:  
+ *data-disno-level.csv*: This file contains the derived data
attributes.  
+ *data-disno-level.gpkg*: This file includes both the data attributes
and simplified polygon(s) of the corresponding administrative unit(s)
(sourced from GADM v 3.6).
...
ph_events <- here('data-output','events','disno-level', 'pct-5-95')
...
process_attributes <- function(disno) {
...
general_attributes <- disnos %>%
  ...
  select(disno) %>%
  pmap_dfr(function(disno) {
    process_attributes(disno)
  })
```
...
# Apply the process_file function to each row in disnos
event_attributes <- disnos %>%
  as.data.frame() %>%
  mutate(geometry = NULL) %>%
  select(disno) %>%
  pmap_dfr(function(disno) {
    process_events(disno)
  })
```
...
st_write(
  output,
  dsn = paste0(ph_out, '/data-disno-level.gpkg'),
  layer = "data-disno-level",
  delete_dsn = TRUE
)
...
write.csv(
  output,
  paste0(ph_out, '/data-disno-level.csv'),
  row.names = F
)
```
