...
knitr::opts_chunk$set(eval = T) # Change to TRUE if chunks should be run when document is knitted.
```
...
pct_levels <- c(10, 90) # Options: c(5,95) or c(10,90)
  ...
  mutate(
    ...
    thres_var = case_when(
      thres_type == 'pct' & dis_type == 'Cold wave' ~ 
        glue('ctn{sprintf("%02d", pct_levels)[1]}_1980_2010'),
      thres_type == 'pct' & dis_type == 'Heat wave' ~ 
        glue('ctx{sprintf("%02d", pct_levels)[2]}_1980_2010'),
      thres_type == 'fixed' & dis_type == 'Cold wave' ~ fixed_variables[1],
      thres_type == 'fixed' & dis_type == 'Heat wave' ~ fixed_variables[2],
      T ~ NA_character_
    ),
...
process_file <- function(disno, type, gadm_gid, gadm_name, gadm_level, analysis_start, analysis_end) {
  ...
  result <- readRDS(file = glue('{ph_in}/{disno}_{gadm_gid}.rds')) %>%
    ...
    summarize(
      ...
      mean_pct10_detrended = round(mean(pull(pick(matches('^ct.*0(?!_1980_2010$)', perl = TRUE)))), 2),
      mean_pct05_detrended = round(mean(pull(pick(matches('^ct.*5(?!_1980_2010$)', perl = TRUE)))), 2),
      
      mean_pct10_1980_2010 = round(mean(pull(pick(matches('^ct.*0_1980_2010$')))), digits = 2),
      mean_pct05_1980_2010 = round(mean(pull(pick(matches('^ct.*5_1980_2010$')))), digits = 2),
...
for (i in 1:length(files)){
  ...
  if(type_i == "Heat wave"){
    file_i <- file_i %>%
      rename(mean_pct90_1980_2010 = mean_pct10_1980_2010,
             mean_pct95_1980_2010 = mean_pct05_1980_2010,
             mean_pct90_detrended = mean_pct10_detrended,
             mean_pct95_detrended = mean_pct05_detrended
      ) %>%
      select(
        disno,
        subtype,
        ...
        event_start,
        event_end,
        mean_pct90_detrended,
        mean_pct95_detrended,
        mean_pct90_1980_2010,
        mean_pct95_1980_2010,
        duration,
        persondays,
        magnitude,
        mean_t,
    ...
    if(pct_levels[2]==90){file_i <- file_i %>% select(-mean_pct95_detrended, -mean_pct95_1980_2010)}
    if(pct_levels[2]==95){file_i <- file_i %>% select(-mean_pct90_detrended, -mean_pct90_1980_2010)}
  ...
  if(type_i == "Cold wave"){
    file_i <- file_i %>%
      select(
        ...
        event_start,
        event_end,
        mean_pct10_detrended,
        mean_pct05_detrended,
        mean_pct10_1980_2010,
        mean_pct05_1980_2010,
        duration,
        persondays,
        magnitude,
        mean_t,
    ...
    if(pct_levels[1]==10){file_i <- file_i %>% select(-mean_pct05_detrended, -mean_pct05_1980_2010)}
    if(pct_levels[1]==5){file_i <- file_i %>% select(-mean_pct10_detrended, -mean_pct10_1980_2010)}
