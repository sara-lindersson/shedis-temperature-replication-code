---
title: 'Attributes at the subdivision-level'
author: 'Sara Lindersson'
date: '`r Sys.Date()`'
output: github_document
---

This script derives attributes at the administrative _subdivision_ unit level for each disaster number _disno_. The outputs from this script include:  
+ `shedis_heatwaves_subdivision.csv`: This file contains the derived data attributes for the heat wave disasters.  
+ `shedis_heatwaves_subdivision.gpkg`: This file includes both the data attributes and a simplified polygon of the corresponding administrative subdivision.  
+ `shedis_coldwaves_subdivision.csv`: This file contains the derived data attributes for the cold wave disasters.  
+ `shedis_coldwaves_subdivision.gpkg`: This file includes both the data attributes and a simplified polygon of the corresponding administrative subdivision.  
...
process_attributes <- function(disno, gadm_gid, analysis_start, analysis_end) {
  ...
  df_zonal <- df_zonal %>%
    summarize(
      ...
      across(
        matches("^ct"),              # select all columns starting with "ct"
        mean,                        # apply mean
        na.rm = TRUE,                # handle missing values safely
        .names = "mean_{.col}"       # create new column names
      )
    )
...
shedis_coldwaves_subdivision <- shedis_coldwaves_subdivision %>%
  ...
  rename(mean_ctn10_detr = mean_ctn10,
         mean_ctn05_detr = mean_ctn05)
...
shedis_heatwaves_subdivision <- shedis_heatwaves_subdivision %>%
  ...
  rename(mean_ctx90_detr = mean_ctx90,
         mean_ctx95_detr = mean_ctx95)
